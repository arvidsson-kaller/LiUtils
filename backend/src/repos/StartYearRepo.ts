...
async function create(year: StartYearInitializer): Promise<StartYear> {
  try {
    const sql =
...
async function findById(startYearId: StartYearId): Promise<StartYear> {
  const res = await db.query('SELECT * from "StartYear" where id = ($1)', [startYearId]);
